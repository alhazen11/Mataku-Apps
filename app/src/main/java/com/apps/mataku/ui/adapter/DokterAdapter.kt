...
import com.apps.mataku.ui.dokter.DokterClickInterface
...
class DokterAdapter(private val context: Context?) : RecyclerView.Adapter<DokterAdapter.ViewHolder>(){
    private var member: List<Member> = ArrayList()
    private var listner: DokterClickInterface? = null
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val root = (LayoutInflater.from(parent.context).inflate(R.layout.list_dokter, parent, false))
        return ViewHolder(root)
    }

    fun addMember(member: List<Member>?, listner: DokterClickInterface) {
        if (member != null) {
            this.member = member
