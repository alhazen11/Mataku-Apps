...
import kotlinx.android.synthetic.main.activity_katarak.*
...
class KatarakActivity : AppCompatActivity(),KatarakView {
    override fun showKatarat(status: String?, error: Boolean, result: String) {}
    @Inject
    lateinit var presenter: KatarakPresenter
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_katarak)
        (applicationContext as Apps).createKatarakComponent().inject(this)
        db = SQLiteHandler(applicationContext)
        user= db!!.userDetails
        idUser= user.get("uid").toString()
        ui_back.setOnClickListener { startActivity(Intent(this@KatarakActivity, DashboardActivity::class.java)) }
    ...
    private fun previewCapturedImage() {
        try {
            ...
            if (lengthbmp >= 7) {
                Toast.makeText(this@KatarakActivity, "Max Ukuran Photo 6MB", Toast.LENGTH_SHORT).show()
            }
            lastBitmap = bitmap
            Glide.with(this).load(lastBitmap).into(ui_foto)
            uploadImage()
    ...
    private fun uploadImage(){
        ...
        val call = apiService.uploadKatarak(image)
            ...
            override fun onResponse(call: Call<DeepResponse>, response: retrofit2.Response<DeepResponse>?) {
               Log.d("test",response?.body().toString())
                if (response != null) {
                    ...
                    if (hasil == 0) {
                        ui_hasil.setText("Tidak terdeteksi katarak")
                        presenter.setViewKatarak(mContext,createQuery(idUser,"Tidak terdeteksi katarak"))
                    } else {
                        ui_hasil.setText("Terdeteksi katarak")
                        presenter.setViewKatarak(mContext,createQuery(idUser,"Terdeteksi katarak"))
                    }
                }
            }
        })
        ...
        builder.setTitle("Permissions required!")
                ...
                .setPositiveButton("GOTO SETTINGS", object : DialogInterface.OnClickListener {
                    override fun onClick(dialog: DialogInterface, which: Int) {
                        CameraUtils.openSettings(this@KatarakActivity)
                    }
                })
